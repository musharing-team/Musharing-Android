...
/**
 * <h1>退出当前用户登录的请求与响应解析任务</h1>
 *
 * 若解析成功调用 {@code listener.onSuccess(String result)}，把一个标志请求成功得的字符串传出给listener
 */
public class LogoutTask extends RequestTask<String> {

    public LogoutTask(RequestTaskListener<String> listener) {
        super(listener);
    }
    ...
    protected String doInBackground(String... strings) {
        try {
            String responseText = RequestUtil.logout(strings[0]);   // uid
