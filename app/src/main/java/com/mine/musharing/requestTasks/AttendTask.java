...
/**
 * <h1>拉取用户加入当前Room的请求与响应解析任务</h1>
 *
 * 若解析成功调用 {@code listener.onSuccess(String result)}，把一个标志请求成功得的字符串传出给listener
 */
public class AttendTask extends RequestTask<String> {

    public AttendTask(RequestTaskListener<String> listener) {
        super(listener);
    }
    ...
    protected String doInBackground(String... strings) {
        try {
            String responseText =  RequestUtil.attend(strings[0], strings[1]);   // uid, target name
